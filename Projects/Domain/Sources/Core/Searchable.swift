        ...
        if isOnlyInitial(word: text) {
            if initialMatching {
                for i in 0...(dataArray.count - 1) {
                    let convertedData = convertToInitial(word: dataArray[i].searchString)
                    
                    if convertedData.starts(with: text) {
                        idxSetPriority.insert(i)
                    }
                    
                    if dataArray[i].searchString.starts(with: text) {
                        idxSetPriority.insert(i)
                    }
                }
            } else {
            ...
            if partialMatching {
                for i in 0...(dataArray.count - 1) {
                    let convertedData = convertToPartial(word: dataArray[i].searchString)
