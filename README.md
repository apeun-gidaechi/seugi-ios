# 스기
![Platforms](https://img.shields.io/badge/Platforms-iOS-red?style=flat-square)

## 🔥 Tech Stack
- SwiftUI
- [Moya](https://github.com/Moya/Moya)
- [Nuke](https://github.com/kean/Nuke)
- [Swinject](https://github.com/Swinject/Swinject)
- [Tuist](https://github.com/tuist/tuist)
- [apeun-stomp-kit](https://github.com/apeun-gidaechi/apeun-stomp-kit)
- [ScopeKit](https://github.com/hhhello0507/ScopeKit)
- [realm-swift](https://github.com/realm/realm-swift)
- MVVM
- Clean Architecture

## 🏃‍♂️ Run Project

### Files
- Add `./XCConfig/Config.xcconfig`
- Add `./Projects/App/Seugi-iOS/Resources/GoogleService-Info.plist`

### Project
- Tusit version: 4.28.1
- [Use mise to install](https://docs.tuist.io/en/guides/quick-start/install-tuist)

just `make go` !
```bash
make go
```
